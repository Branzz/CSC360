...
const Product = class Product {
    ...
    constructor(productString) {
        ...
        this.name = productSplit[0];
        this.price = Number(productSplit[1]);
        this.availability = productSplit[2];
    }
     ...
     */
    static inStock = products => products.filter(p => p.availability == "In Stock")
     ...
     * Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array 
     */
    static halfOff = products => products.map(p => new Product(p.name + "," + (p.price * 0.5) + "," + p.availability))
     ...
     */
    static printProducts = products => products.forEach(p => console.log("Product: " + p.name
             + ", Cost: " + p.price.toLocaleString('en-US', {  style: 'currency', currency: 'USD', })
             + ", Availability: " + (p.availability == "In Stock" ? "Yes" : "No")))
